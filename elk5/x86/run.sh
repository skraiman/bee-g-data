...
docker run --name elastic -p 9200:9200  -v "$DIR/elasticsearch-data":/usr/share/elasticsearch/data -d  elasticsearch:5.1.2
...
docker run --name grafana --link elastic:elasticsearch  -d -p 3000:3000 -v "$DIR/grafana-data":/var/lib/grafana -e "GF_INSTALL_PLUGINS=mtanda-histogram-panel,grafana-piechart-panel,grafana-worldmap-panel" grafana/grafana
